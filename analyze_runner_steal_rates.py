...
（見 cpbl_decision_model_*_summary.json 的 by_hitter_lineup）。若把跑者成功率拿去
比對單一全季中位數，等於丟掉這個核心發現。因此改成逐棒次比對：每位跑者的季成功率
分別對照 1–9 棒各自的門檻，標出換到哪些棒次值得跑——判斷永遠是「跑者 × 當下棒次」
的組合，不是跑者一個人就能決定的單一標籤。
