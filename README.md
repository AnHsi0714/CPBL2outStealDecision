...
|---|---|---|---|---|---|---|---|---|---|
| 門檻 | 53.4% | 53.9% | 56.0% | **61.3%** | 59.0% | 57.2% | 57.5% | 58.8% | 59.8% |

門檻幾乎完全由「V不跑－V失敗」這個分子決定，「V成功－V失敗」（盜壘成功能省下多少）在各棒次都差不多（約 32%–34%），不隨棒次變。分子大小才是關鍵，拆解如下：

- **第 1、2 棒門檻最低（53%–54%）**：不管盜壘失敗被保留、還是不跑正常出局後換下一棒，接手的都是實力相近的早段棒次，兩種情況差距小，是全場相對最適合冒險盜壘的棒次。
- **第 4 棒門檻最高（61.3%）**：主因是「這位打者自己夠強」——續打時常能靠自己的打擊在本半局多得分，但盜壘刺會立刻結束半局，等於白白放棄這個打席的價值。這是「打者自身能力」驅動的效應。
- **第 9 棒門檻次高（59.8%）**：原因跟第 4 棒不同，是「保留效應」——正常情況下第 9 棒出局後，下一局通常輪到全隊最強的第 1 棒開路；但盜壘刺會讓第 9 棒自己被保留、下一局改由他先打，等於賠掉了讓第 1 棒提前登場的機會。這是「打序被延後一棒」驅動的效應，也是本研究最初設定要驗證的核心機制。
- **第 5–8 棒門檻居中（57%–59%）**：實力較平均的一段，銜接的下一棒實力也差不多，「打者自身能力」跟「保留效應」都不明顯。

換句話說，門檻高不是單一原因，而是「這位打者自己多強」跟「盜壘失敗會不會卡住更強棒次的機會」兩股力量疊加，且兩者在打線的不同位置各自主導——第 4 棒是前者主導、第 9 棒是後者主導，不能用同一套理由解釋。

長打型／選球型打者（PA ≥ 100，聯盟中位數切分）的比較：

| 分組 | 高長打 | 低長打 | 高選球 | 低選球 |
|---|---|---|---|---|
| 門檻中位數 | 61.1%（n=957） | 54.8%（n=757） | 59.8%（n=899） | 56.9%（n=815） |
...
|---|---|---|---|---|---|
| BBpct_proxy（選球力，3.2 節公式） | 保送/觸身率 | 0.329 | 59.8%（n=899） | 56.9%（n=815） | < 0.001 |
| OBP_proxy（真上壘率） | 安打率+保送觸身率 | 0.455 | 57.8%（n=1027） | 59.4%（n=687） | 0.001 |
| **SingleRate_proxy（純接觸型，1.4 節概念）** | 單打率 | **-0.379** | **54.5%（n=869）** | **62.6%（n=845）** | **< 0.001** |

三個指標方向不一致：**選球力愈高，門檻愈高**（更不該跑）；但**真上壘率愈高，門檻反而愈低**（更該跑），效應主要在後段棒次；把上壘率拆解到最乾淨的**純單打率**後，方向更明確、差距也最大（8.2 個百分點，是所有分組比較裡最大的），且與 ISO 是負相關而非正相關，代表這是三者中跟長打力切得最乾淨的一個。
